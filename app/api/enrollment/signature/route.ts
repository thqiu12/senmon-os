...
export const GET = withTenant(async (request: NextRequest) => {
  try {
    ...
    if (applicationId) {
      ...
      const signature = await db.enrollmentSignature.findFirst({ where: { applicationId } });
    ...
    if (applicationNo && email) {
      ...
      const signature = await db.enrollmentSignature.findFirst({ where: { applicationId: ownership.applicationId } });
      return NextResponse.json({ signature: signature || null });
    }
...
export const POST = withTenant(async (request: NextRequest) => {
  try {
    ...
    const db = getTenantDb();
    // 入学手続きが存在するか確認
    const proc = await db.enrollmentProcedure.findFirst({ where: { applicationId: ownership.applicationId } });
    if (!proc) {
      return NextResponse.json({ error: "入学手続きが開始されていません" }, { status: 400 });
    }

    const signature = await db.enrollmentSignature.upsert({
