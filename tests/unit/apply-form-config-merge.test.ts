...
// ConfigRow ファクトリ（既定は「学校共通(applicantType null)・有効」）。
// 全校共通(schoolId null) スコープは廃止されたため、既定 schoolId は実在の学校キーにする。
...
describe("mergeFormConfig — 出願者タイプ別の有効/無効", () => {
  it("日本人: 学校共通(null applicantType)で在留資格が有効でも、日本人フォームでは非表示（共通行が日本人の既定オフを上書きしない）", () => {
  ...
  it("日本人: 全在日情報フィールドが学校共通(true)でも一括で非表示になる", () => {
  ...
  it("日本人: 学校×日本人 行で明示的に有効化すれば表示される（管理者が個別に出せる）", () => {
    const rows = [
  ...
  it("留学生: 学校共通で在留資格が有効なら表示", () => {
    const rows = [row({ fieldKey: "residenceStatus", applicantType: null, isEnabled: true })];
    const out = mergeFormConfig(FORM_FIELD_DEFAULTS, rows, "foreign");
    expect(has(out, "residenceStatus")).toBe(true);
  });

  it("留学生: 管理者が学校共通で在留資格を無効化したら非表示（共通の無効化は尊重）", () => {
  ...
  it("日本人: 学校×日本人で明示的に無効化したら非表示", () => {
    const rows = [
      row({ fieldKey: "residenceStatus", applicantType: "japanese", schoolId: "kanagawa-judo", isEnabled: false }),
    ];
    const out = mergeFormConfig(FORM_FIELD_DEFAULTS, rows, "japanese");
    expect(has(out, "residenceStatus")).toBe(false);
  });

  it("通常項目: 学校共通で無効化すればどのタイプでも非表示（共通の無効化は全タイプに効く）", () => {
  ...
  it("custom select の options がマージ出力に伝播する", () => {
    const rows = [row({ fieldKey: "custom_color", applicantType: null, schoolId: "chuo-seminar", isEnabled: true, options: "赤\n青\n緑" } as any)];
    const out = mergeFormConfig(FORM_FIELD_DEFAULTS, rows, "foreign");
  ...
  it("学校×日本人 が 学校×共通 を上書きする（優先順位の確認）", () => {
    const rows = [
      row({ fieldKey: NORMAL_KEY, applicantType: null, schoolId: "kanagawa-judo", isEnabled: true, label: "共通ラベル" }),
      row({ fieldKey: NORMAL_KEY, applicantType: "japanese", schoolId: "kanagawa-judo", isEnabled: false }),
    ];
    const out = mergeFormConfig(FORM_FIELD_DEFAULTS, rows, "japanese");
...
// 本番状態(管理画面で全デフォルトが学校共通・有効として保存済み)を再現し、
...
describe("mergeFormConfig — 各フォームが正常動作するか（総合）", () => {
  ...
  it("学校共通行のラベル変更は両フォームに反映される（編集が効く）", () => {
...
describe("mergeFormConfig — showWhenExamMode 伝播", () => {
  it("条件付きカスタム項目の showWhenExamMode がマージ出力に伝播する", () => {
    const rows = [row({ fieldKey: "custom_aoreason", isEnabled: true, showWhenExamMode: "em_ao", section: "選考区分" } as any)];
    const out = mergeFormConfig(FORM_FIELD_DEFAULTS, rows, "foreign");
